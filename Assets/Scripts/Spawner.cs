...
public class Spawner : MonoBehaviour
{
    ...
    void Update()
    {
        GameManager gm = scoreEntity.GetComponent<GameManager>();
        if (gm.mCurrentScore <= 50f)
        {
            spawnFrequencyMean = 4f - (0.065f * gm.mCurrentScore);
        }
        else if(gm.mCurrentScore >= 75f)
        {
            spawnFrequencyMean = 0.75f;
        }
        else
        {
            spawnFrequencyMean = 0.5f;
        }
    ...
    public void SpawnObstacle()
    {
        ...
        if (gm.mCurrentScore <= 50f)
        {
            obstacleScript.movementSpeed = 1.0f + (0.08f * gm.mCurrentScore);
        }
        else
        {
            obstacleScript.movementSpeed = 5f;
        }
